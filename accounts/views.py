...
from .serializers import (
    ...
    UserManagementSerializer, UserSetPasswordSerializer,
...
class UserDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        if serializer.instance.id == self.request.user.id:
            # Prevent deactivating oneself or removing own admin role
            if serializer.validated_data.get('is_active') is False:
                from rest_framework.exceptions import ValidationError
                raise ValidationError({"error": "You cannot deactivate your own account."})
            if serializer.validated_data.get('role') and serializer.validated_data['role'] != 'admin':
                from rest_framework.exceptions import ValidationError
                raise ValidationError({"error": "You cannot remove your own admin privileges."})
        super().perform_update(serializer)


class UserSetPasswordView(APIView):
    """POST /api/users/<id>/set-password/ — securely reset user password"""
    permission_classes = [IsAuthenticated, IsAdminRole]
